...
export const UserSummary = ({ selectedCurrency }) => {
  ...
  const _unsettledGains = unsettledGains ? weiToCurrency(unsettledGains) : 0.0;
  const _unsettledKITTY = unsettledKITTY ? unsettledKITTY : 0.0;
      ...
      <div className="px-2 md:px-4">
        <div className="flex items-center">
          ...
          <dl className="px-4 md:px-8 py-3 lg:pl-12 bg-white rounded-3xl lg:rounded-r-3xl">
            <dt className="leading-none whitespace-no-wrap">Unclaimed Gains</dt>
            <dd className="text-xl lg:text-2xl font-black text-green-500 leading-none">
              {_unsettledGains.toFixed(2)} {selectedCurrency}
            </dd>
          </dl>
        </div>
      </div>
