...
export const Stake = () => {
	...
	const handleAllowance = async (val) => {
		...
		if ( active && account && _walletBalance > 0 && val > 0 && val <= _walletBalance ) {

			const amount = currencyToWei(val + _userAllowance);
	...
	const handleStake = async (val) => {
		...
		if ( active && account && _userAllowance > 0 && val > 0 && val <= _userAllowance ) {
		...
		<>
			...
			<div className="my-2">
				<Button
					...
					isDisabled={_walletBalance === 0 || value === 0}
				>
					Approve
				</Button>
            </div>
